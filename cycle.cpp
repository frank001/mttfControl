...
void Cycle::start() {
    ...
    m_TmrVibrate.start((m_VibrateStart-1)*1000);    //TODO: get rid of the minus 1 second.
    m_Tmr5mlux.start((m_5mluxStart-1)*1000);
    m_Tmr50lux.start((m_50luxStart-1)*1000);

    m_TmrTubes.start((m_TubesStart-1)*1000);

    m_bCycle = true;

    m_TmrCycle.start(m_CycleDuration*1000);
...
void Cycle::tmrTubesTimeout(){
    ...
    if (!m_bTubes) {
        ...
        m_bTubes = true;
        m_TmrTubes.start((m_TubesDuration*1000)-100);
    } else {
...
void Cycle::tmrVibrateTimeout(){
    ...
    if (!m_bVibrate) {
        ...
        m_bVibrate = true;
        m_TmrVibrate.start((m_VibrateDuration*1000)-100);
    } else {
...
void Cycle::tmr5mluxTimeout() {
    ...
    if (!m_b5mlux) {
        ...
        m_b5mlux = true;
        m_Tmr5mlux.start(m_5mLuxDuration*1000);
    } else {
...
void Cycle::tmr50luxTimeout() {
    ...
    if (!m_b50lux) {
        ...
        m_b50lux = true;
        m_Tmr50lux.start(m_50LuxDuration*1000);
    } else {
        message(COMMAND|WATCH, "1mlux on.");
        m_Command->HandleRaw("setLight","1");
        m_b50lux = false;
